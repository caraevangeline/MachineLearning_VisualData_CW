...
for ii = 1:nimages
      ...
      %calculate the histogram (that should be a vector with the same dimensionality as the number of codewords and normalize it
          [minv,index] = min(d);
...
idright = right_v(21);% set number you are about to visualize
idwrong = wrong_v(7);% set number you are about to visualize, be sure it is nor larger than length of wrong_v 
...
figure, 
subplot(2,4,1),imshow(imread(strcat('image/',image_names{idright+300}))),title('test image'),axis off;
subplot(2,4,2),bar(BoW(idright+300,:)),xlim([1,500]),title('BoW of test image');
subplot(2,4,3),imshow(imread(strcat('image/',image_names{NNresult(idright)}))),title('Right Label NN image'),axis off;
subplot(2,4,4),bar(BoW(NNresult(idright),:)),xlim([1,500]),title('BoW of NN image');

subplot(2,4,5),imshow(imread(strcat('image/',image_names{idwrong+300}))),title('test image'),axis off;
subplot(2,4,6),bar(BoW(idwrong+300,:)),xlim([1,500]),title('BoW of test image');
subplot(2,4,7),imshow(imread(strcat('image/',image_names{NNresult(idwrong)}))),title('Wrong Label NN image'),axis off;
...
% figure 
% for k = 1:nv
%     %subplot(nrows,5,2*k-1),imshow(imread(strcat('image/',image_names{right_v(k)+300}))),title(strcat('PL-right',class_names{predict_label(right_v(k))})),axis tight,axis off;
%     subplot(nrows,5,2*k),imshow(imread(strcat('image/',image_names{wrong_v(k)+300}))),title(strcat('PL-wrong-',class_names{predict_label(wrong_v(k))})),axis tight,axis off;
% end
% n_w = length(right_v);
% nv = min(n_w,20);
